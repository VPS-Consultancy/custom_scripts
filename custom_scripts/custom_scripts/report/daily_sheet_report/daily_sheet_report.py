...
def get_data(filters):
	...
	pe_list_rc = frappe.db.sql('''select %s as inward_voucher_type, pe.name as voucher_no,
					...
					where pe.payment_type = "Receive" and (pe.mode_of_payment ="Wire Transfer" or pe.mode_of_payment ="Credit Card" or pe.mode_of_payment ="Google Pay" or pe.mode_of_payment ="Cash") and pe.posting_date 
					between %s and %s and pe.docstatus = 1''',
					('Payment Entry',filters['cf_date'],filters['cf_date']),  as_dict = True)

	cust_pe_list_pay = frappe.db.sql('''select %s as expense_type, pe.name as ex_voucher_no, 
					pe.paid_amount as ex_amount, pe.remarks as ex_remarks,
					pe.mode_of_payment as ex_payment_mode
					from `tabPayment Entry` pe 
					where pe.payment_type = "Pay" and pe.party_type = 'Customer' and (pe.mode_of_payment ="Wire Transfer" or pe.mode_of_payment ="Credit Card" or pe.mode_of_payment ="Google Pay" or pe.mode_of_payment ="Cash") and pe.posting_date  
					between %s and %s and pe.docstatus = 1''',
					('Payment Entry',filters['cf_date'],filters['cf_date']),  as_dict = True)
	
	common_pe_list_pay = frappe.db.sql('''select %s as expense_type, pe.name as ex_voucher_no, 
					pe.paid_amount as ex_amount, pe.remarks as ex_remarks,
					pe.mode_of_payment as ex_payment_mode
					from `tabPayment Entry` pe 
					where pe.payment_type = "Pay" and (pe.party_type = 'Supplier' or pe.party_type = 'Employee') and pe.mode_of_payment ="Cash" and pe.posting_date  
					between %s and %s and pe.docstatus = 1''',
					('Payment Entry',filters['cf_date'],filters['cf_date']),  as_dict = True)
