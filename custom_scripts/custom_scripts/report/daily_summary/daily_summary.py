...
def get_data(filters):
	...
	si_cash_type = frappe.db.sql("""select %s as inward_voucher_type, si.name as voucher_no,
					...
					where si.invoice_type = "Cash Invoice" and (sip.mode_of_payment ="Wire Transfer" or sip.mode_of_payment ="Credit Card" or sip.mode_of_payment ="Google Pay" or sip.mode_of_payment ="Cash" or sip.mode_of_payment = "Cheque" or sip.mode_of_payment = "Kotak Card Swiping") and si.posting_date 
	...
	return_si = frappe.db.sql("""select %s as inward_voucher_type, si.name as voucher_no,
					...
					from `tabSales Invoice` si join `tabSales Invoice Payment` sip
					on sip.parent = si.name
					where si.invoice_type = "Cash Return" and (sip.mode_of_payment ="Wire Transfer" or sip.mode_of_payment ="Credit Card" or sip.mode_of_payment ="Google Pay" or sip.mode_of_payment ="Cash" or sip.mode_of_payment = "Cheque" or sip.mode_of_payment = "Kotak Card Swiping") and si.posting_date 
	...
	pe_list_rc = frappe.db.sql('''select %s as inward_voucher_type, pe.name as voucher_no,
					...
					where pe.payment_type = "Receive" and (pe.mode_of_payment ="Wire Transfer" or pe.mode_of_payment ="Credit Card" or pe.mode_of_payment ="Google Pay" or pe.mode_of_payment ="Cash" or pe.mode_of_payment = "Card Swiping" or pe.mode_of_payment = "Cheque" or pe.mode_of_payment = "Kotak Card Swiping") and pe.posting_date 
	...
	cust_pe_list_pay = frappe.db.sql('''select %s as inward_voucher_type, pe.name as voucher_no, 
					...
					pe.mode_of_payment as in_payment_mode, pe.party_type, pe.party
					from `tabPayment Entry` pe 
					where pe.payment_type = "Pay" and pe.party_type = 'Customer' and (pe.mode_of_payment ="Wire Transfer" or pe.mode_of_payment ="Credit Card" or pe.mode_of_payment ="Google Pay" or pe.mode_of_payment ="Cash" or pe.mode_of_payment = "Card Swiping" or pe.mode_of_payment = "Cheque" or pe.mode_of_payment = "Kotak Card Swiping") and pe.posting_date  
					between %s and %s and pe.docstatus = 1''',
					('Payment Entry',filters['cf_date'],filters['cf_date']),  as_dict = True)
	...
	common_pe_list_pay = frappe.db.sql('''select %s as inward_voucher_type, pe.name as voucher_no, 
					...
					pe.mode_of_payment as in_payment_mode, pe.party_type, pe.party
					from `tabPayment Entry` pe 
					where pe.payment_type = "Pay" and (pe.party_type = 'Supplier' or pe.party_type = 'Employee') and (pe.mode_of_payment ="Cash" or pe.mode_of_payment = "Card Swiping" or pe.mode_of_payment = "Cheque" or pe.mode_of_payment = "Kotak Card Swiping") and pe.posting_date  
					between %s and %s and pe.docstatus = 1''',
					('Payment Entry',filters['cf_date'],filters['cf_date']),  as_dict = True)
