...
def get_data(filters):
	...
	if 'cf_sales_person' not in filters:
		si_pe_list = frappe.db.sql('''select "Sales Invoice" as invoice_type, si.name as invoice, si.base_grand_total - si.outstanding_amount as received_amount, si.status as status
		...
		pos_je_list = frappe.db.sql('''select "POS Invoice" as invoice_type, pi.name as invoice , pi.paid_amount as received_amount, pi.status as status
			sp.sales_person, sp.incentives
		...
		filtered_sales_invoice = frappe.db.sql(
			'''select "Sales Invoice" as invoice_type, si.name as invoice, si.base_grand_total - si.outstanding_amount as received_amount, sp.sales_person, sp.incentives, si.status as status
		...
		filtered_pos_invoice = frappe.db.sql(
			'''select "POS Invoice" as invoice_type, pi.name as invoice , pi.paid_amount as received_amount, sp.sales_person, sp.incentives, pi.status as status
