...
def get_rows(
	...
	for employee, details in employee_details.items():
		...
		if filters.summarized_view:
			...
			if not attendance:
				continue
			# frappe.msgprint(f""" {str(attendance)}""")
...
def get_attendance_summary_and_days(employee: str, filters: dict):
	# Define SQL queries for summary and days
	summary_query = """
		SELECT
			SUM(CASE WHEN status IN ('Present', 'Work From Home') THEN 1 ELSE 0 END) AS total_present,
			SUM(CASE WHEN status = 'Absent' THEN 1 ELSE 0 END) AS total_absent,
			SUM(CASE WHEN status = 'On Leave' THEN 1 ELSE 0 END) AS total_leaves,
			SUM(CASE WHEN status = 'Half Day' THEN 0.5 ELSE 0 END) AS total_half_days,
	...
	days_query = """
		SELECT DISTINCT EXTRACT(DAY FROM attendance_date) AS day_of_month
		FROM `tabAttendance`
		WHERE docstatus = 1
			AND employee = %s
			AND company = %s
	...
	# Execute the queries
	summary = db.sql(summary_query, (employee, filters["company"], filters["month"], filters["year"]), as_dict=True)
	days = db.sql_list(days_query, (employee, filters["company"], filters["month"], filters["year"]))
	# frappe.msgprint(str(summary))
